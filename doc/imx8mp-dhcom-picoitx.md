i.MX8M Plus DHCOM SoM on DH PicoITX carrier board
=================================================
...
```
host$ zstd -d dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic.zst
```
...
host$ bmaptool copy --bmap \
    dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic.bmap \
    dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic.zst \
    /dev/sdx
bmaptool: info: block map format version 2.0
bmaptool: info: 400474 blocks of size 4096 (1.5 GiB), mapped 246005 blocks (961.0 MiB or 61.4%)
bmaptool: info: copying image 'dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic' to block device '/dev/sdx' using bmap file 'dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic.bmap'
...
```
host$ dd if=dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic \
         of=/dev/sdx bs=4M
```
...
bmaptool: info: block map format version 2.0
bmaptool: info: 400474 blocks of size 4096 (1.5 GiB), mapped 246005 blocks (961.0 MiB or 61.4%)
bmaptool: info: copying image 'dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic' to block device '/dev/mmcblk2' using bmap file 'dh-image-demo-dh-imx8mp-dhcom-picoitx.rootfs.wic.bmap'
bmaptool: info: 100% copied
...
installation into SPI NOR can be performed from within U-Boot itself, from
SD/eMMC card, or from running Linux userspace.
...
```
root@dh-imx8mp-dhcom-picoitx:~# udevadm info /dev/mtdblock0
...
```
root@dh-imx8mp-dhcom-picoitx:~# dd if=flash-fcfb.bin of=/dev/mtdblock0
3840+0 records in
3840+0 records out
...
```
root@dh-imx8mp-dhcom-picoitx:~# reboot
```
...
Make sure the system is powered off and USB A-to-A cable is connected to port
`USB A` `X10`. Remove microSD card from the microSD card slot located on
...
Use `uuu` to send U-Boot `flash.bin` binary using SDPS via USB A to the SoC:
...
The following Device Tree Overlays are currently supported:
 ...
 - DH 626-100 adapter card with Chefree CH101OLHLWH-002 LVDS display attached to it.
   `fdt-imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo`
...
11001952 bytes read in 116 ms (90.5 MiB/s)
```
...
## Checking Image at 50000000 ...
   FIT image found
   FIT description: Kernel fitImage for DH Linux Distribution/6.12.56+git/dh-imx8mp-dhcom-picoitx
   Created:         2025-10-29  13:09:02 UTC
    Image 0 (kernel-1)
     ...
     Hash algo:    sha256
     Hash value:   c29302ef6d23b81b040eb29bce831ed84c94847076a768e5c988dfe90555b815
    Image 1 (fdt-freescale_imx8mp-dhcom-picoitx.dtb)
     Description:  Flattened Device Tree blob
     Created:      2025-10-29  13:09:02 UTC
     Type:         Flat Device Tree
     Compression:  uncompressed
     Data Start:   0x50a68ee4
     Data Size:    78387 Bytes = 76.5 KiB
     Architecture: AArch64
     Load Address: 0x5c000000
     Hash algo:    sha256
     Hash value:   ea1a2c324ecc6938c1d890a607e6709009e09e025768c2dc24078d47c0b76204
    Image 2 (fdt-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo)
     Description:  Flattened Device Tree blob
     Created:      2025-10-29  13:09:02 UTC
     Type:         Flat Device Tree
     Compression:  uncompressed
     Data Start:   0x50a7c238
     Data Size:    6079 Bytes = 5.9 KiB
     Architecture: AArch64
     Load Address: 0x5c080000
     Hash algo:    sha256
     Hash value:   f97c9f2445f49d7eb472af70336e7290167e90e316b2b4ed70d8d68501c257b6
    Default Configuration: 'conf-freescale_imx8mp-dhcom-picoitx.dtb'
    Configuration 0 (conf-freescale_imx8mp-dhcom-picoitx.dtb)
     Description:  1 Linux kernel, FDT blob
     Kernel:       kernel-1
     FDT:          fdt-freescale_imx8mp-dhcom-picoitx.dtb
     Hash algo:    sha256
     Hash value:   unavailable
    Configuration 1 (conf-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo)
     Description:  0 FDT blob
     Kernel:       unavailable
     FDT:          fdt-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo
     Hash algo:    sha256
     Hash value:   unavailable
## Checking hash(es) for FIT Image at 50000000 ...
   Hash(es) for Image 0 (kernel-1): sha256+
   Hash(es) for Image 1 (fdt-freescale_imx8mp-dhcom-picoitx.dtb): sha256+
   Hash(es) for Image 2 (fdt-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo): sha256+
```
...
To configure the system to use `imx8mp-dhcom-picoitx.dtb` base Device
Tree and apply additional `imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo`
...
```
                     ...
                     |                                       |
                     v                                       v
=> env set loaddtos '#conf-freescale_imx8mp-dhcom-picoitx.dtb#conf-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo'
                       ^                                       ^
                       |                                       |
                       |                                       '--- 626-100 display DTO
                       |
...
```
u-boot=> env set loaddtos '#conf-freescale_imx8mp-dhcom-picoitx.dtb#conf-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo'
u-boot=> boot
switch to partitions #0, OK
mmc0 is current device
Scanning mmc 0:1...
Found U-Boot script /boot/boot.scr
6667 bytes read in 0 ms
## Executing script at 50000000
11001952 bytes read in 116 ms (90.5 MiB/s)
Using base DT fdt-freescale_imx8mp-dhcom-picoitx.dtb
## Copying 'fdt-freescale_imx8mp-dhcom-picoitx.dtb' subimage from FIT image at 50000000 ...
sha256+    Loading part 253 ... OK
Working FDT set to 5c000000
Applying DTO fdt-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo
## Copying 'fdt-freescale_imx8mp-dhcom-picoitx-overlay-626-100-x2-ch101olhlwh.dtbo' subimage from FIT image at 50000000
 ...
sha256+    Loading part 253 ... OK
...
## Loading kernel (any) from FIT Image at 50000000 ...
   Using 'conf-freescale_imx8mp-dhcom-picoitx.dtb' configuration
...
Working FDT set to 5c000000
   ...
   Loading Device Tree to 00000000ade53000, end 00000000adea9fff ... OK
Working FDT set to ade53000
...
header `X5`, this is `serial@30860000`. Other serial ports can be
made available by modifying U-Boot control Device Tree.

Linux on this system provides serial console access via 2x5 pin
header `X5`, this is `30860000.serial`. Linux also provides one
additional serial port accessible via display connector `X2`,
`30a60000.serial` and one RS485 port on external connector `X8`
`30880000.serial` .
...
```
root@dh-imx8mp-dhcom-picoitx:~# udevadm info /dev/ttymxc*
...
```
root@dh-imx8mp-dhcom-picoitx:~# echo 1 > /sys/class/leds/yellow\:indicator/brightness
```

To disable an LED, write 0 into matching sysfs LED attribute:
```
root@dh-imx8mp-dhcom-picoitx:~# echo 0 > /sys/class/leds/yellow\:indicator/brightness
```
...
GPIO1_5: output: 0 [x] led-0.gpios
...
```
u-boot=> gpio set GPIO1_5
gpio: pin GPIO1_5 (gpio 5) value is 1
```

To set a GPIO as Output-Low, use `gpio clear <pin>` command:
```
u-boot=> gpio clear GPIO1_5
gpio: pin GPIO1_5 (gpio 5) value is 0
```

To toggle a GPIO, use `gpio toggle <pin>` command:
```
u-boot=> gpio toggle GPIO1_5
gpio: pin GPIO1_5 (gpio 5) value is 1
u-boot=> gpio toggle GPIO1_5
gpio: pin GPIO1_5 (gpio 5) value is 0
```
...
gpiochip0 - 32 lines:
        ...
        line   5:       "DHCOM-I"               output consumer="yellow:indicator"
        line   6:       "PicoITX-HW0"           input
        line   7:       "PicoITX-HW2"           input
        line   8:       "DHCOM-B"               output active-low consumer="reset"
...
```
root@dh-imx8mp-dhcom-picoitx:~# cat /sys/devices/platform/soc\@0/30800000.bus/30b50000.mmc/mmc_host/mmc*/mmc*/block/mmcblk*/dev
179:96
^^^ ^^

root@dh-imx8mp-dhcom-picoitx:~# ls -la /dev/mmcblk*
...
...
```
root@dh-imx8mp-dhcom-picoitx:~# cat /sys/devices/platform/soc\@0/30800000.bus/30b60000.mmc/mmc_host/mmc*/mmc*/block/mmcblk*/dev
179:0
^^^ ^
...
```
u-boot=> net list
eth0 : ethernet@30bf0000 00:11:22:33:44:55 active
```
...
```
root@dh-imx8mp-dhcom-picoitx:~# ip addr show
...
...
```
root@dh-imx8mp-dhcom-picoitx:~# udevadm info /sys/class/net/ethsom0
...
```
root@dh-imx8mp-dhcom-picoitx:~# udevadm info /sys/class/net/wlansom0
...
```
root@dh-imx8mp-dhcom-picoitx:~# ip link set wlansom0 up
root@dh-imx8mp-dhcom-picoitx:~# iw dev wlansom0 scan
...
```
root@dh-imx8mp-dhcom-picoitx:~# bluetoothctl
...
USB XHCI 1.10
Bus usb@38200000: 2 USB Device(s) found
...
```
root@dh-imx8mp-dhcom-picoitx:~# lsusb
